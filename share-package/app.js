...
function render() {
  ...
  renderModelComparison(filteredRows);
}
...
function renderEmptyState() {
  ...
  dom.modelChart.innerHTML = emptyCard("Model comparison will appear here.");
  dom.modelTable.innerHTML = "";
}
...
function renderNoMatches() {
  ...
  dom.campaignTable.innerHTML = "";
  dom.modelChart.innerHTML = emptyCard("Try widening the campaign, model, or date filters.");
  dom.modelTable.innerHTML = "";
}
...
function renderKpis(rows) {
  ...
  const totalBrands = uniqueValues(rows.map((row) => row.brand)).length;
  const avgCtr = safeAverage(rows.map((row) => row.ctr));

  const cards = [
    { label: "Total Spend", value: formatValue(totals.spend, "currency"), note: `${distinctMonths || 0} tracked months` },
    { label: "Impressions", value: formatValue(totals.impressions, "integer"), note: `${totalBrands || 0} loaded brands` },
    { label: "Landing Page Views", value: formatValue(totals.landingPageViews, "integer"), note: "From filtered rows" },
...
function renderTrendSection(rows) {
  ...
  const sourceRows = rows.filter((row) => hasMetricData(row, ["impressions", "landingPageViews", "spend"]));
  ...
  dom.trendChart.innerHTML = ordered.length
    ? ordered.length === 1
      ? singleMetricProfileChart({
          ...
          points: [
            { label: "Impressions", value: ordered[0].impressions, kind: "integer", color: "#3d7ee8" },
            { label: "Landing page views", value: ordered[0].landingPageViews, kind: "integer", color: "#e74c3c" },
            { label: "Amount spent (CAD)", value: ordered[0].spend, kind: "currency", color: "#f4b400" },
          ],
        })
      : multiSeriesChart({
          labels: ordered.map((entry) => formatChartLabel(entry.label, chartDimension)),
          leftSeries: [
            { label: "Impressions", color: "#3d7ee8", kind: "integer", values: ordered.map((entry) => entry.impressions) },
            { label: "Landing page views", color: "#e74c3c", kind: "integer", values: ordered.map((entry) => entry.landingPageViews) },
          ],
          rightSeries: [{ label: "Amount spent (CAD)", color: "#f4b400", kind: "currency", values: ordered.map((entry) => entry.spend) }],
        })
    : emptyCard("No dated rows are available for the selected filters.");

  dom.trendTable.innerHTML = ordered.length
    ? tableHtml(
        [chartDimension === "monthKey" ? "Month" : "Campaign Type", "Impressions", "Landing page views", "Amount spent (CAD)"],
        ordered.map((entry) => [
          formatChartLabel(entry.label, chartDimension),
          formatValue(entry.impressions, "integer"),
          formatValue(entry.landingPageViews, "integer"),
          formatValue(entry.spend, "currency"),
        ])
      )
    : "";
}
...
function renderCampaignBreakdown(rows) {
  ...
  const sourceRows = rows.filter((row) =>
    hasMetricData(row, ["subscriptions", "addsToCart", "costPerSubscription", "costPerAddToCart"])
  );
  const ordered = aggregateForCharts(sourceRows, chartDimension).map((entry) => ({
      label: entry.label,
      subscriptions: entry.metrics.subscriptions,
      addsToCart: entry.metrics.addsToCart,
      costPerSubscription: entry.metrics.costPerSubscription,
  ...
  dom.campaignBreakdown.innerHTML = ordered.length
    ? ordered.length === 1
      ? singleMetricProfileChart({
          seriesLabel: formatChartLabel(ordered[0].label, chartDimension),
          points: [
            { label: "Sum of Subscriptions", value: ordered[0].subscriptions, kind: "integer", color: "#3d7ee8" },
            { label: "Sum of Adds to cart", value: ordered[0].addsToCart, kind: "integer", color: "#e74c3c" },
            { label: "Average of Cost per subscription (CAD)", value: ordered[0].costPerSubscription, kind: "currency", color: "#f4b400" },
            { label: "Average of Cost per add to cart (CAD)", value: ordered[0].costPerAddToCart, kind: "currency", color: "#2a9d8f" },
          ],
        })
      : multiSeriesChart({
          labels: ordered.map((entry) => formatChartLabel(entry.label, chartDimension)),
          leftSeries: [
            { label: "Sum of Subscriptions", color: "#3d7ee8", kind: "integer", values: ordered.map((entry) => entry.subscriptions) },
            { label: "Sum of Adds to cart", color: "#e74c3c", kind: "integer", values: ordered.map((entry) => entry.addsToCart) },
          ],
          rightSeries: [
            { label: "Average of Cost per subscription (CAD)", color: "#f4b400", kind: "currency", values: ordered.map((entry) => entry.costPerSubscription) },
            { label: "Average of Cost per add to cart (CAD)", color: "#2a9d8f", kind: "currency", values: ordered.map((entry) => entry.costPerAddToCart) },
          ],
        })
    : emptyCard("Subscription and add-to-cart values were not found in the loaded files.");

  dom.campaignTable.innerHTML = ordered.length
    ? tableHtml(
        [chartDimension === "monthKey" ? "Month" : "Campaign Type", "Sum of Subscriptions", "Sum of Adds to cart", "Average of Cost per subscription (CAD)", "Average of Cost per add to cart (CAD)"],
        ordered.map((entry) => [
          formatChartLabel(entry.label, chartDimension),
          formatValue(entry.subscriptions, "integer"),
          formatValue(entry.addsToCart, "integer"),
          formatValue(entry.costPerSubscription, "currency"),
...
function renderModelComparison(rows) {
  ...
  dom.modelChart.innerHTML = ordered.length
    ? ordered.length === 1
      ? singleMetricProfileChart({
          seriesLabel: formatChartLabel(ordered[0].label, chartDimension),
          points: [
            { label: "Average of CPM (cost per 1,000 impressions) (CAD)", value: ordered[0].cpm, kind: "currency", color: "#3d7ee8" },
            { label: "Average of CTR (link click-through rate)", value: ordered[0].ctr, kind: "percent", color: "#e74c3c" },
            { label: "Average of Cost per landing page view (CAD)", value: ordered[0].costPerLandingPageView, kind: "currency", color: "#f4b400" },
          ],
        })
      : multiSeriesChart({
          labels: ordered.map((entry) => formatChartLabel(entry.label, chartDimension)),
          leftSeries: [{ label: "Average of CTR (link click-through rate)", color: "#e74c3c", kind: "percent", values: ordered.map((entry) => entry.ctr) }],
          rightSeries: [
            { label: "Average of CPM (cost per 1,000 impressions) (CAD)", color: "#3d7ee8", kind: "currency", values: ordered.map((entry) => entry.cpm) },
            { label: "Average of Cost per landing page view (CAD)", color: "#f4b400", kind: "currency", values: ordered.map((entry) => entry.costPerLandingPageView) },
          ],
        })
    : emptyCard("CPM, CTR, and landing page efficiency values were not found in the loaded files.");

  dom.modelTable.innerHTML = ordered.length
    ? tableHtml(
        [chartDimension === "monthKey" ? "Month" : "Campaign Type", "Average of CPM (cost per 1,000 impressions) (CAD)", "Average of CTR (link click-through rate)", "Average of Cost per landing page view (CAD)"],
        ordered.map((entry) => [
          formatChartLabel(entry.label, chartDimension),
...
function aggregateByDimension(rows, key) {
  ...
  return Array.from(grouped.entries()).map(([label, groupedRows]) => ({
    label,
    metrics: sumMetrics(groupedRows),
  }));
}
  ...
  return rows.reduce(
    (totals, row) => {
      metricCatalog.forEach((metric) => {
        totals[metric.key] += row[metric.key] || 0;
      });
      return totals;
    },
    Object.fromEntries(metricCatalog.map((metric) => [metric.key, 0]))
  );
}
...
function multiSeriesChart({ labels, leftSeries, rightSeries }) {
  ...
  const legend = `
    ...
    <div class="axis-note">Left axis: ${escapeHtml(leftSeries.map((series) => series.label).join(" / "))} | Right axis: ${escapeHtml(rightSeries.map((series) => series.label).join(" / "))}</div>
  `;
