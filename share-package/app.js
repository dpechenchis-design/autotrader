...
async function loadStaticManifestFiles() {
  ...
  return Promise.all(
    fileEntries.map(async (entry) => {
      ...
      const url = typeof entry === "string" ? `./data/${encodeURIComponent(entry)}` : entry.url || `./data/${encodeURIComponent(entry.name)}`;
...
function applyFilters(rows) {
  return rows.filter((row) => {
    ...
    if (state.filters.language !== "All" && row.language !== state.filters.language) {
      return false;
    }
...
function getRowsForFilterOptions(excludedKey) {
  return state.normalizedRows.filter((row) => {
    ...
    if (excludedKey !== "language" && state.filters.language !== "All" && row.language !== state.filters.language) {
      return false;
    }
